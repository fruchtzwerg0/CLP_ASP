module CLP.unifyDisunifyOld where
...
unifyDisunify : 
  ...
  → A
  → List (List (Σᵢ 𝒞 (ℒ ∘ Code) Code Constraint ⊎ Σᵢ 𝒞 (Dual ∘ Constraint) Code Constraint) × A)
unifyDisunify witness occurs apply unifications new other = 
  unifyDisunify₀ witness occurs apply rules [] ((catMaybes ∘ Data.List.map (λ {(inj₁ x) → just x ; (inj₂ _) → nothing})) (new ++ unifications)) other [] []
